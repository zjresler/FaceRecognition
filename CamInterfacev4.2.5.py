...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        #start of new code
        #widgets for taking multiple video clips

        #setup widgets to get number of clips for multiple videos
        Label(self, text = "Number of videos:").grid(row = 4, column = 3, sticky = W+E+N+S)
        self.vm_num = Entry(self, width = 5)
        self.vm_num.grid(row = 5, column = 3 )
          
        #setup widgets to get frequency of clips for multiple videos
        Label(self, text = "Length of Videos in Seconds:").grid(row = 6, column = 3, sticky = W+E+N+S)
        self.vm_secs = Entry(self, width = 6)
        self.vm_secs.grid(row = 7, column = 3)



        #setup widgets to get directory name for videos
        Label(self, text = "Video Directory Name :").grid(row = 2, column = 3, sticky = W+E+N+S)
        self.vm_filename = Entry(self)
        self.vm_filename.grid(row = 3, column = 3)
        

        # create 'Start Video multiple' button
        Button( self,
                text = "Start video clips",
                command = self.action_multiple_video,
                bg='green',
                ).grid(row=8, column = 3, sticky = W+E+N+S)
       
        # create text field to display results
        self.results_txt = Text(self, height = 16, wrap = WORD)
    ...
    #.................... Method: action_multiple_video ................
    # This method takes multiple videos
    # and updates the status display to show the names of the saved video files
    #
    def action_multiple_video(self):
        status = ""
        numvideos = 1
        numsecs = 1
        chosenfile = ""
        ...
        #first get filename
        contents= self.vm_filename.get()
        self.chosenfile= str(contents)
        status = "chosen directory name : " + self.chosenfile + "\n"

        try:
            # first create the directory to store the videos in
            os.mkdir(self.chosenfile)
        except:
           # can't create the directory
           status += "Invalid file namechosen - " + chosenfile + "\n"
        ...
        if ferror == 0 :
           # have created directory ok so
           # next get no of videos
            contents = self.vm_num.get()            
            try:
                self.numvideos = int(contents)
            except:
                self.numvideos = 1
                status = status + "Invalid Number of videos chosen - Set at 1.\n"
            ...
            if self.numvideos<1 :
                ...
                status = status + "Number of videos chosen : " + str(self.numvideos) + "\n"

            # now get no of seconds between videos
            contents = self.vm_secs.get()            
            try:
                self.numsecs = int(contents)
            except:
                # invalid number format
            ...
            # now take the videos
            try:
                for i, filename in enumerate(self.camera.record_sequence(self.chosenfile+'/video%02d.h264' % i for i in range(self.numvideos))):
                    self.camera.wait_recording(self.numsecs)                    
                    #sleep(self.numsecs)        old code
                    status += 'captured video %s' % filename + "\n"
                    if i==(self.numvideos-1):
                        break
            except:
               status += "Invalid file namechosen - " + chosenfile + "\n"
    ...
    def setup_camera(self):
        ...
        self.camera.preview_window = (50,200,600,500)
