...
def train(train_dr, model_save_path=None, num_neighbors=None, knn_alg='ball_tree'):
    ...
    Y = []
    z = 0
    cpt = sum([len(files) for r, d, files in os.walk(train_dr)])
...
def train_from_vid(video, name, model_save_path=None, num_neighbors=None, knn_alg='ball_tree'):
    ...
    while True:
        ...
        z += 1
        print("Reading frame {} / {}".format(z, length))
    ...
    # converted_rgb_frame = frame[:, :, ::-1]
        ...
        face_bounding_boxes = hog(frame)
        if len(face_bounding_boxes) != 1:
            print("Frame {} not sutable for training: {}".format(z, "Didn't find a face" if len(face_bounding_boxes) < 1 else "Found more than one face"))
        else:
            X.append(face_encodings(frame, known_faces_locations=face_bounding_boxes)[0])
            Y.append(name)
     # set the neighbor amount        
    if num_neighbors is None:
        num_neighbors = int(round(math.sqrt(len(X))))
...
def recog(X_Ipath, knn_classifier=None, model_path=None, distance=0.4):
    ...
    if matches and closest[0][0][0] <= distance:
        ...
        acc.append([knn_classifier.predict(face_encod), 1 - closest[0]])
        
    return [[(pre, loc) if rec else ("unknown", loc) for pre, loc, rec in zip(knn_classifier.predict(face_encod), X_face_loc, matches)], acc]


# Displays the image with the name added to the bounding box
def show_known_face_name(image_path, predictions, acc):
    ...
    for name, (top, right, bottom, left) in predictions:
     ...
     #      save = input("----------- Please enter a valid respose to save this image or not? (y/n): ")
    ...
    save_images(image_path.split('.')[0], the_image)
     
     # just cleans up the memory usage a bit
    del draw


def recog_from_vid(X_Ipath, knn_classifier=None, model_path=None, distance=0.4):
     ...
     # mode='RGB'
     # the_image = X_image.convert(mode)
     X_face_loc = hog(X_image)
...
def show_known_face_name_from_vid(image, predictions, acc):
